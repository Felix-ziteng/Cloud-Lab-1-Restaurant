...
export default function TabletOrderingCompact({ orderId, tokenKind, tableNumber }: { orderId: string; tokenKind: string; tableNumber: string }) {
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {qty > 0 ? (
                          <div className="flex items-center gap-2.5">
                            <button type="button" onClick={() => addToCart(dish.id, -1)} className="flex size-[42px] items-center justify-center rounded-full bg-[oklch(94%_0.01_40)] text-lg font-bold text-[oklch(45%_0.02_30)]">
                              −
                            </button>
                            <span className="min-w-3.5 text-center text-base font-bold">{qty}</span>
                            <button type="button" onClick={() => addToCart(dish.id, 1)} className="flex size-[42px] items-center justify-center rounded-full bg-[oklch(60%_0.21_35)] text-lg font-bold text-white">
                              +
                            </button>
                          </div>
                        ) : (
                          <button type="button" onClick={() => addToCart(dish.id, 1)} className="rounded-full bg-[oklch(90%_0.05_45)] px-5 py-2 text-sm font-bold text-[oklch(45%_0.18_35)]">
                            加入
                          </button>
                        )}
                      </div>
              ...
              <div className="flex flex-col gap-3.5">
                {Object.keys(cart).length === 0 ? (
                  ...
                  Object.entries(cart).map(([dishId, qty]) => {
                    ...
                    if (!dish) return null;
                    return (
                      <div key={dishId} className="flex items-center justify-between gap-2 text-sm">
                        <div>
                          <p className="font-bold">{dish.name}</p>
                          <p className="text-[oklch(50%_0.02_40)]">× {qty}</p>
                        </div>
                        <span className="font-bold text-[oklch(58%_0.2_35)]">¥{(Number(dish.price) * qty).toFixed(2)}</span>
                      </div>
                    );
                  })
                )}
